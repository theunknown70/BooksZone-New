...
const Home = () => {
  ...
  const searchBook = () => {
    if (search.trim() || tags) {
      ...
      history.push(`/books/search?searchQuery=${search || 'none'}&tags=${tags.join(',')}`);
    } else {
      history.push('/');
    }
  };
        ...
        <Grid container justify="space-between" alignItems="stretch" spacing={3} className={classes.gridContainer}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <AppBar className={classes.appBarSearch} position="static" color="inherit">
              <TextField onKeyDown={handleKeyPress} name="search" variant="outlined" label="Search By Price" fullWidth value={search} onChange={(e) => setSearch(e.target.value)} />
              <ChipInput
                style={{ margin: '10px 0' }}
                value={tags}
